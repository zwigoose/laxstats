...
export default function FieldMapInput({ onZoneSelected }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: 16 }}>
      <div style={{ fontSize: 14, fontWeight: 600, color: C.gray750 }}>Which zone was the shot taken from?</div>
      <div
        style={{
          width: '100%',
          maxWidth: 420,
          aspectRatio: '120/110',
          border: `2px solid ${C.emerald800}`,
          borderRadius: 8,
          overflow: 'hidden',
          boxShadow: `inset 0 2px 8px ${C.blackA08}`,
        }}
      >
        <svg viewBox="0 0 120 110" style={{ width: '100%', height: '100%', display: 'block', userSelect: 'none' }}>
          {/* Grass */}
          <rect x="0" y="0" width="120" height="110" fill={C.emerald400} fillOpacity="0.2" />
          {/* Sidelines */}
          <line x1="0" y1="0" x2="0" y2="110" stroke={C.emerald800} strokeWidth="1" />
          <line x1="120" y1="0" x2="120" y2="110" stroke={C.emerald800} strokeWidth="1" />
          {/* End line */}
          <line x1="0" y1="110" x2="120" y2="110" stroke={C.emerald800} strokeWidth="2" />
          {/* Midline */}
          <line x1="0" y1="0" x2="120" y2="0" stroke={C.emerald800} strokeWidth="1.5" />
          ...
          <line x1="0" y1="40" x2="120" y2="40" stroke={C.emerald800} strokeWidth="0.75" strokeDasharray="3,2" opacity="0.8" />
          {/* Behind-goal area — not a valid shot origin */}
          <rect x="0" y="90" width="120" height="20" fill={C.emerald800} fillOpacity="0.18" />
          <text x="60" y="103" textAnchor="middle" fontSize="3.5" fill={C.emerald800} opacity="0.5">Behind goal — not a shot</text>
          {/* Goal crease */}
            ...
            return (
              <g key={zone} onClick={() => handleTap(zone)} style={{ cursor: 'pointer' }}>
                <rect
                  ...
                  fill={active ? C.emerald800 : C.emerald400}
                  fillOpacity={active ? 0.5 : 0.12}
                  stroke={C.emerald800} strokeWidth="0.6" strokeOpacity="0.55"
                />
                <text
                  x={col.x + col.w / 2} y={band.y + band.h / 2 + 2}
                  textAnchor="middle" fontSize="6" fontWeight="700"
                  fill={C.emerald800} opacity={active ? 0.9 : 0.35}
      ...
      <button
        ...
        style={{ padding: '8px 20px', fontSize: 13, fontWeight: 500, background: C.gray50, border: `1px solid ${C.gray250}`, borderRadius: 20, cursor: 'pointer', color: C.gray650 }}
      >
        Skip Location
      </button>
    </div>
