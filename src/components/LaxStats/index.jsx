...
export default function LaxStats({
  ...
  const [videoOffset, setVideoOffset] = useState(0); // Hudl/Film sync offset in seconds
  ...
  function handleExportHudl() {
    const headers = ["Time", "Quarter", "Event", "Team", "Player"];
    const rows = log.map(e => {
      const absTime = absElapsedSecs(e.quarter, toSecs(e.goalTime || e.timeoutTime || "0:00"));
      const videoTime = absTime + videoOffset;
      const m = Math.floor(videoTime / 60);
      const s = Math.floor(videoTime % 60);
      const timeStr = `${m}:${s.toString().padStart(2, '0')}`;
      const p = e.player ? `#${e.player.num} ${e.player.name}` : "";
      return [timeStr, qLabel(e.quarter), e.event, teams[e.teamIdx]?.name, p];
    });
    ...
    link.setAttribute("download", `laxstats_hudl_${new Date().toISOString().slice(0, 10)}.csv`);
        ...
        <div>
          ...
          {statsTab === "film" && (
            <div style={{ padding: 20 }}>
              <h3>🎥 Film Integration (Hudl/Krossover)</h3>
              <p style={{ fontSize: 14, color: "#666" }}>
                Sync the game clock with your video file to export a CSV that can be imported as "clips" or "tags".
              </p>
              
              <div style={{ background: "#f8f9fa", padding: 15, borderRadius: 8, marginBottom: 20 }}>
                <label style={{ display: "block", marginBottom: 10, fontWeight: 500 }}>Video Offset (seconds)</label>
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <input 
                    type="number" 
                    value={videoOffset} 
                    onChange={(e) => setVideoOffset(parseInt(e.target.value) || 0)}
                    style={{ padding: "8px 12px", borderRadius: 4, border: "1px solid #ddd", width: 100 }}
                  />
                  <span style={{ fontSize: 12, color: "#888" }}>
                    Adjust if the video starts before/after the game whistle.
                  </span>
                </div>
              </div>

              <button 
                onClick={handleExportHudl}
                style={{ 
                  background: "#1a6bab", 
                  color: "white", 
                  padding: "12px 24px", 
