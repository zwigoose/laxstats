...
export default function GameTimeline({ scoringTimeline, teams, teamColors, compact = false }) {
  ...
  const thStyle   = { padding: compact ? "4px 8px" : "6px 8px 6px 14px", fontSize: 11, fontWeight: 600, color: C.gray400, textAlign: "left", borderBottom: `1px solid ${C.gray75}`, whiteSpace: "nowrap" };
  const thStyleR  = { ...thStyle, textAlign: "right", paddingLeft: 8, paddingRight: compact ? 8 : 14 };
  const tdStyle   = { padding: cellPad, verticalAlign: "top", borderBottom: `1px solid ${C.gray40}`, fontSize: compact ? 12 : 14 };
  const tdStyleR  = { ...tdStyle, textAlign: "right", padding: cellPadR };

  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: compact ? 12 : 14, fontFamily: F.ui }}>
      <thead>
        <tr>
      ...
      <tbody>
        {rows.map((entry, i) => {
          ...
          if (entry.type === "timeout") {
            ...
            return (
              <tr key={i} style={{ background: C.gray25 }}>
                <td style={tdStyle}>
                  <div style={{ fontWeight: 600, fontSize: timeFont, fontVariantNumeric: "tabular-nums" }}>{to.timeoutTime || "—"}</div>
                  <div style={{ fontSize: qFont, fontWeight: 600, color: C.gray500, marginTop: 1 }}>{qLabel(to.quarter)}</div>
                </td>
                <td style={{ ...tdStyle, paddingLeft: 8 }}>
                  <span style={{ color: teamColors[to.teamIdx], fontWeight: 500 }}>{teams[to.teamIdx]?.name}</span>
                </td>
                <td style={{ ...tdStyle, paddingLeft: 8, color: C.gray500, fontStyle: "italic" }} colSpan={2}>⏸ Timeout</td>
                {scoreCell}
              </tr>
            );
          }
          ...
          if (entry.type === "penalty") {
            ...
            return (
              <tr key={i} style={{ background: C.orange25 }}>
                <td style={tdStyle}>
                  <div style={{ fontWeight: 600, fontSize: timeFont, fontVariantNumeric: "tabular-nums" }}>{pen.penaltyTime || "—"}</div>
                  <div style={{ fontSize: qFont, fontWeight: 600, color: C.gray500, marginTop: 1 }}>{qLabel(pen.quarter)}</div>
                </td>
                <td style={{ ...tdStyle, paddingLeft: 8 }}>
                  <span style={{ color: teamColors[pen.teamIdx], fontWeight: 500 }}>{teams[pen.teamIdx]?.name}</span>
                </td>
                <td style={{ ...tdStyle, paddingLeft: 8, color: C.gray500, fontStyle: "italic" }}>{penLabel}</td>
          ...
          return (
            <tr key={i}>
              <td style={tdStyle}>
                ...
                <div style={{ fontSize: qFont, fontWeight: 600, color: C.gray500, marginTop: 1 }}>{qLabel(goal.quarter)}</div>
              </td>
              <td style={{ ...tdStyle, paddingLeft: 8 }}>
              ...
              <td style={{ ...tdStyle, paddingLeft: 8 }}>
                ...
                {goalCount > 1 && <span style={{ marginLeft: 5, color: C.gray400, fontSize: compact ? 11 : 13 }}>({goalCount})</span>}
              ...
              <td style={{ ...tdStyle, paddingLeft: 8 }}>
                {assist
                  ? <span style={{ color: C.gray500 }}>#{assist.player?.num} {assist.player?.name}</span>
                  : <span style={{ color: C.gray250 }}>—</span>}
              </td>
              {scoreCell}
            </tr>
          );
