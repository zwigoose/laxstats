...
- **`game_meta_events`** — legacy quarter-transition table, kept only as transition scaffolding: a forwarding trigger copies inserts into the stream with deterministic ids, serving stale pre-Phase-3 browser bundles and drained legacy offline-queue ops. **Deferred cleanup**: a future migration drops this table, its forwarding trigger, the `games` state-projection trigger, and `insertMetaEvent` — write it only once production has run the event-sourced client long enough that no stale bundles remain (staging: any time after refreshing open tabs)
